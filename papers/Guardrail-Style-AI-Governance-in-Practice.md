...
The dominant paradigm in AI governance today is "barrier-style"—setting boundaries, classifying risks, and conducting post-hoc audits. This paper proposes an alternative path: **"guardrail-style governance."** Unlike barriers that say "no entry," guardrails say "you can drive freely within this range, but you cannot fall off." Rules are short, hard, observable, and verifiable constraints—not lengthy procedural manuals. Using the author's independently designed and maintained SOLO/TAFA framework as a case study, this paper reports four months of engineering experience operating an AI governance system comprising six iron laws, five principles, four privilege rings, a three-tier separation of powers, and a ten-agent pipeline deployed in a public hospital disciplinary inspection setting. The core finding: the operability of guardrail-style governance depends on three conditions—executable constraints (not merely readable rules), immediate feedback loops (not periodic audits), and observable degradation signals (not reliance on human judgment). This paper provides concrete implementation methods and a record of operational failures.
...
A sharper question: **the complexity of governance frameworks backfires on governance effectiveness.** When rules are so numerous they require specialized training to understand, implementers subconsciously ignore them—a phenomenon we call the "rule inflation trap." It is like posting fifty warning signs on a highway: drivers end up reading none of them.

This paper's starting point is an engineering intuition: **good governance rules should be guardrails, not walls.** Walls say "this entire area is off-limits"—safe but useless. Guardrails say "you can operate freely within this range, but you cannot fall off"—enabling capability while controlling risk. This intuition has been validated, refined, and crystallized into an operable system through four months of engineering practice.

This paper's contribution is not proposing yet another governance framework, but reporting on **a system that is already running:** the complete chain from rule design to engineering implementation to operational evolution.

---

## 2. Overview of the SOLO/TAFA System
...
Feedback between layers is structural: capability layer output → audit layer evaluation → authority layer rule revision → capability layer re-execution. This is not a post-hoc remedy of "audit findings, then fix"—it is a cycle built into the design from the start.

### 2.2 Deployment Context

The system operates in a public hospital disciplinary inspection context, covering four domains:
...
- **319 Regulatory Documents:** Full-text search in local Wiki
...
After four months of iteration, we have crystallized six executable constraints—each a "guardrail": short, hard, observable, and verifiable.
...
**Operationalization:** Desensitization scripts + grep verification + pre-upload review. In July 2026, it was discovered that `git rebase` can silently restore pre-desensitization content. A "re-run grep immediately after rebase" guardrail was added.
...
**Operationalization:** The DI pipeline deploys Agent 1a (Wiki full-text search) → Agent 1b (Peking University Law Database version verification) → Agent 1c (three-way merge), locking in regulatory original text and version before generating any qualitative conclusions.
...
**Rule:** Only clean up temporary files you yourself generated.
...
**Operationalization:** Two dimensions—cognitive humility (admit ignorance without shortcuts) + architectural humility (understand before simplifying; mark uncertainty with [UNCERTAIN] when incomplete). Real-world case: Docker sandbox configuration change was declared "done" without verification. After correction, end-to-end verification gates were added.
...
This is the most important meta-rule in the entire guardrail system. Its logic: rule increase ≠ governance improvement. When a problem occurs, before adding any rule, ask—**"Were the existing rules actually followed?"**

### 4.2 Inflation Detection Algorithm

The SOLO audit system includes automated inflation detection:
...
- **2026-05-20:** SOUL.md expanded from 180 lines to 350 lines (+94%), with zero increase in violations during the same period → triggered P0 audit
- **Audit result:** 46% of new content was "re-emphasis" of existing rules—no new information
- **Resolution:** Restored to original version. Necessary additions were moved to skills rather than the kernel.
...
The disciplinary review pipeline is the best embodiment of guardrail philosophy at the engineering level. Ten agents run sequentially, with each agent's output serving as the gate check for the next:
...
The most innovative component is Phase 8: upon pipeline completion, automatically scan lesson logs and use the `skill_manage` tool to instantly patch agent protocol files for P0-level issues. From discovery to repair—requiring 30 days on OpenClaw (waiting for monthly cron)—takes less than one minute on Hermes.

### 5.4 Live Validation

On July 19, 2026, the DI pipeline completed its first live run on the Hermes platform:
...
Unlike traditional "model fine-tuning" or "prompt optimization," the SOLO system uses skill protocol files (SKILL.md) to store reusable rules. When a DI case discovers a new issue, the response is not modifying code or models, but—
...
Learned: Item-by-item allegation-response structure, "investigation found → investigation team determined" evidence chain, "do not omit procedural deficiencies" principle

Action: Revised draft.md and revise.md agent protocol files, adding 7 new investigation report standard checklist items

Effective: Immediate

**Case 2: Regulation-Manager Search Chain Fix**

Input: Regulation search skipped local Wiki and went directly to OCR-ing PDF

Learned: Mandatory search chain: Wiki (rg) → Peking University Law Database → government websites → third-party

Action: Added mandatory search chain sequence section to regulation-manager/SKILL.md

### 6.3 Guardrails on Evolution

Self-repair is itself constrained by guardrails—Phase 8's decision matrix only allows P0 + high-confidence fixes to execute automatically. Low-confidence fixes require human confirmation. This prevents the risk of "fixes introducing problems worse than the original."
...
One code-level gate outweighs 656 readable rules. Our experience: every rule should be automatically verifiable by tools within 3 seconds. "Accepting gifts is a violation of discipline" is less useful than—
...
The repair cycle in the OpenClaw era was 30 days (dependent on monthly cron). In the Hermes era, it is reduced to 1 minute (skill_manage instant patch). This speed improvement is not merely "faster"—it transforms "post-hoc patching" into "real-time immunity."

### 7.3 Observable Degradation Signals Over Human Judgment Reliance

The inflation detection algorithm does not require human judgment about "which rules are redundant"—it only compares the rate of change between rule additions and violation additions. Degradation signals appear in the data, not in subjective experience.
...
| Cost control | ❌ | ❌ 500M tokens today | ✅ 4-tier routing saves 80% |
...
The core argument of this paper: **AI governance should not be merely a "policy document" problem—it should be an "engineering implementation" problem.** Guardrail-style governance differs from barrier-style governance not in the quantity or coverage of rules, but in—
...
Four months of operation have validated the feasibility of these three conditions. Next steps include: quantitative evaluation of AI agent performance, intelligent routing optimization for token consumption, and validation of the guardrail paradigm's generalizability across additional domains (such as compliance review and hospital inspection).
...
[7] Shi Bing, "TAFA Ternary Separation: A Third Approach to Public Hospital AI Governance," 2026
[8] Hermes Agent Documentation, 2026

---

*This paper is based on real-world AI governance engineering practice. All technical details (DI pipeline, SOLO audit, MEV Five Layers, 655 Six Iron Laws) have been validated through four months of operation. Data current as of July 19, 2026.*
